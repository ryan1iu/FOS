...
#define PTSIZE \
    (PGSIZE * NPTENTRIES)  // bytes mapped by a page directory entry 4MB
...
#define PTE_W 0x002  // 开启页面读/写功能，默认是只读的
#define PTE_U \
    0x004  // 开启User权限，该页面可以被所有程序访问
           // 默认只有管理员可以访问
#define PTE_PWT 0x008  // 写回策略
#define PTE_PCD 0x010  // 禁用缓存
#define PTE_A \
    0x020  // 用于指示在虚拟地址转换期间是否使用了该entry
           // 更新该位的任务由内核承担
#define PTE_D 0x040   // 页面是否被写入
